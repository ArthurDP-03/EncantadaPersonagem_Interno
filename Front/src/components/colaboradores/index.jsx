...
function FormAdministrador({ dados, onChange, isEditing = false }) {
      ...
      {!isEditing && (
        <label>
          ...
          <input
            ...
            onChange={(e) =>
              onChange({ ...dados, confirmacaoSenha: e.target.value })
            }
          />
        </label>
      )}
...
function FormAtor({ dados, onChange, isEditing = false }) {
  const { t } = useTranslation();
...
function Colaboradores() {
  ...
  function mostrarSenhasDiferentes() {
    Swal.fire({
      icon: "warning",
      title: t("common.validation.title"),
      text: t("common.validation.passwordMismatch"),
    });
  }
  ...
  async function handleCriar(event) {
    ...
    if (tipoForm === "admin") {
      ...
      if (!emailRegex.test(formAdmin.email)) {
        mostrarEmailInvalido();
        return;
      }
      if (formAdmin.senha !== formAdmin.confirmacaoSenha) {
        mostrarSenhasDiferentes();
        return;
      }
      ...
      const dadosAdmin = { ...formAdmin };
      delete dadosAdmin.confirmacaoSenha;

      try {
        setSalvando(true);
        await criarAdmin(dadosAdmin);
        setModalCriar(false);
    ...
    if (!emailRegex.test(formAtor.email)) {
      mostrarEmailInvalido();
      return;
    }
    if (formAtor.senha !== formAtor.confirmacaoSenha) {
      mostrarSenhasDiferentes();
      return;
    }
    ...
    if (
      formAtor.altura === "" ||
      formAtor.altura === null ||
      formAtor.altura === undefined ||
      isNaN(Number(formAtor.altura))
    ) {
    ...
    if (
      formAtor.peso === "" ||
      formAtor.peso === null ||
      formAtor.peso === undefined ||
      isNaN(Number(formAtor.peso))
    ) {
      mostrarValidacao("common.fields.weight", true);
      return;
    }

    const dadosAtor = { ...formAtor };
    delete dadosAtor.confirmacaoSenha;
  ...
  async function handleEditar(event) {
    ...
    if (modalEditar.tipo === "admin") {
      ...
      try {
        setSalvando(true);
        await editarAdmin(modalEditar.dados.id, modalEditar.dados);
        setModalEditar(null);
      } catch (err) {
    ...
    try {
      setSalvando(true);
      await editarAtor(modalEditar.dados.id, {
        ...modalEditar.dados,
  ...
  return (
    <section className="section-colaboradores">
              ...
              {administradoresFiltrados.length === 0 ? (
                ...
                administradoresFiltrados.map((admin) => (
                  <Card_linha
                    ...
                    titulo={admin.nome}
                    informacoes={{
                      [t("common.fields.email")]: admin.email,
                      [t("common.fields.phone")]: admin.telefone,
                    }}
                    onEditar={() =>
                      setModalEditar({
                        tipo: "admin",
      ...
      <button
        ...
        title={t("collaborators.newButton")}
        ...
        <div className="modal">
          <form onSubmit={handleCriar}>
            ...
            {tipoForm === "admin" ? (
              ...
              <FormAtor dados={formAtor} onChange={setFormAtor} isEditing={false} />
            )}
